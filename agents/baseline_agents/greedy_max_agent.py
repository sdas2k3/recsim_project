...
class GreedyMaxAgent(agent.AbstractEpisodicRecommenderAgent):
    """An agent that recommends slates with the highest values (maximizing) based on the first element of document features."""

    def __init__(self,
                 action_space,
                 choice_model=cm.MultinomialLogitChoiceModel({'no_click_mass': 5})):
        """Initializes a new greedy max agent.
        ...
        """
        super(GreedyMaxAgent, self).__init__(action_space)
    ...
    def step(self, reward, observation):
        ...
        # Find the indices of the highest scoring documents
    ...
    def findBestDocuments(self, doc_obs):
        """Returns the indices of the highest scores in sorted order based on the first element of doc_obs values.
        ...
        Returns:
            sorted_indices: A list of integers indexing the highest scores, in sorted order based on the first element of doc.
        """
...
def create_greedy_max_agent(env, choice_model=None):
    """
    Creates an instance of the GreedyMaxAgent.
    ...
    :return: An instance of GreedyMaxAgent.
    """
    return GreedyMaxAgent(action_space=env.action_space)
